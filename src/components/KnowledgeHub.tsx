  ...
  return (
    <section id="knowledge" className="py-12 md:py-20 bg-slate-50 relative overflow-hidden text-right" suppressHydrationWarning>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl relative z-10" suppressHydrationWarning>
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-right transition-all flex flex-col justify-between relative group ${
                    isSelected
                      ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-200 scale-[1.02] ring-2 ring-blue-400/50'
                      : 'bg-white border-slate-200/80 text-slate-700 hover:border-blue-300 hover:shadow-md'
                  }`}
                >
                  ...
                  <div>
                    <h4 className={`font-black text-sm mb-0.5 ${isSelected ? 'text-white' : 'text-slate-900'}`}>
                      {cat.title}
                    </h4>
                    <p className={`text-[11px] leading-tight line-clamp-1 ${isSelected ? 'text-blue-100' : 'text-slate-500'}`}>
                      {cat.subtitle}
                    </p>
                  </div>
                </button>
        ...
        <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200/80 shadow-sm mb-12">
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="relative w-full flex-1">
              ...
              <input 
                ...
                className="w-full pl-10 pr-12 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all placeholder:text-slate-400"
              />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery('')}
                  className="absolute left-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 bg-slate-200 rounded-full"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
            ...
            {(searchQuery || selectedSituation !== 'all') && (
              <button
                ...
                className="px-4 py-3 bg-slate-100 text-slate-600 hover:bg-slate-200 font-bold text-xs rounded-2xl transition-all whitespace-nowrap shrink-0 flex items-center gap-2"
              >
          ...
          <div className="mb-14">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {featuredArticles.map((art) => (
                <div 
                  key={`featured-${art.id}`}
                  className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6 sm:p-7 rounded-3xl shadow-xl border border-slate-700/60 flex flex-col justify-between hover:border-blue-400/50 transition-all group"
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="bg-blue-500/20 text-blue-300 text-xs font-black px-3 py-1 rounded-full border border-blue-400/30 flex items-center gap-1">
                    ...
                    <h4 className="text-xl font-black text-white mb-3 group-hover:text-blue-300 transition-colors leading-snug">
                      {art.title}
                    </h4>

                    {art.keyQuestion && (
                      <div className="bg-slate-800/80 border border-slate-700 p-3 rounded-xl mb-4 text-xs text-blue-200 font-semibold leading-relaxed">
                        ❓ <span className="font-bold text-white">השאלה:</span> {art.keyQuestion}
                      </div>
                    )}
                  ...
                  <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between">
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-md flex items-center gap-2"
                    >
                      <span>{expandedIds.has(art.id) ? 'סגור מאמר' : 'קריאה מהירה'}</span>
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform ${expandedIds.has(art.id) ? 'rotate-180' : ''}`} />
                    </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-2">
              ...
              <span className="text-sm font-bold bg-slate-200 text-slate-700 px-3 py-0.5 rounded-full">
          ...
          {filteredArticles.length === 0 ? (
            ...
            <div className="bg-white border border-slate-200 p-12 rounded-3xl text-center space-y-4 my-8 shadow-sm">
              ...
              <div className="pt-2 flex flex-wrap justify-center gap-3">
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs px-5 py-3 rounded-xl transition-all"
                >
                  הצג את כל המאמרים
                </button>
                <a 
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-5 py-3 rounded-xl transition-all shadow-md flex items-center gap-2"
                >
                  ...
                  return (
                    <motion.article
                      ...
                      className={`bg-white rounded-3xl border transition-all duration-300 overflow-hidden flex flex-col justify-between ${
                        isExpanded 
                          ? 'border-blue-300 shadow-2xl ring-2 ring-blue-100 col-span-1 md:col-span-2' 
                      ...
                      <div className="p-6 sm:p-8 flex flex-col h-full">
                        ...
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex items-center gap-3">
                            <span className="w-12 h-12 bg-blue-50/80 flex items-center justify-center rounded-2xl border border-blue-100 shrink-0">
                              {getArticleIcon(article.id)}
                            </span>
                            <div>
                              <span className="text-blue-600 font-bold text-xs block">
                                {article.category}
                              </span>
                          ...
                          <span className="bg-slate-100 text-slate-600 text-[11px] font-bold px-2.5 py-1 rounded-full shrink-0">
                        ...
                        {article.keyQuestion && (
                          <div className="bg-blue-50/60 border-r-4 border-blue-600 p-3.5 rounded-xl mb-4 text-xs sm:text-sm text-slate-800 font-semibold leading-relaxed">
                            <span className="text-blue-700 font-black block mb-0.5">❓ השאלה המרכזית:</span>
                            {article.keyQuestion}
                          </div>
                        )}

                        {/* Value Trigger / Teaser */}
                        <div className="bg-amber-50/60 border border-amber-200/60 p-3.5 rounded-xl mb-6 text-xs sm:text-sm text-slate-700 leading-relaxed">
                          <span className="text-amber-900 font-bold block mb-1">💡 מה תגלו במאמר?</span>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100 gap-3">
                          <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs uppercase transition-all ${
                              isExpanded 
                          ...
                          <Link 
                            href={`/${article.slug}`}
                            className="text-slate-500 hover:text-blue-600 text-xs font-bold flex items-center gap-1 transition-colors px-3 py-2 rounded-lg hover:bg-slate-50"
                          >
                          ...
                          {isExpanded && (
                            ...
                            >
                              <div className="pt-8 mt-8 border-t border-slate-200 space-y-8">
                                ...
                                <div className="prose prose-blue max-w-none prose-lg text-slate-700 leading-relaxed">
                                  {article.content}
                                </div>

                                {/* Author E-E-A-T Bio Box */}
                                <div className="p-6 bg-slate-50 border border-slate-200/80 rounded-2xl flex flex-col sm:flex-row gap-5 items-center sm:items-start text-right">
                                ...
                                <div className="bg-emerald-50 border border-emerald-200 p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
                                  ...
                                  <a 
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-6 py-3 rounded-xl transition-all shadow-md shrink-0 flex items-center gap-2"
                                  >
                                    <MessageSquare className="w-4 h-4" />
                                ...
                                <div className="pt-4 flex justify-center">
                                  <button
                                    ...
                                    className="text-slate-400 hover:text-blue-600 font-bold text-xs flex items-center gap-2 transition-colors group"
                                  >
