...
const cityContent: Record<string, CityData> = {
  'תל אביב': {
    ...
    desc: 'בדק בית בתל אביב: ביקורת הנדסית מותאמת למלאי הדיור הוותיק בעיר, לפרויקטי תמ"א 38 ולאתגרי בנייה צפופה. אבחון ליקויי בנייה, רטיבות ובעיות איטום ע"י מהנדס מומחה מאריקס ביקורת מבנים.',
    articleHeadline: 'בדק בית בתל אביב: ביקורת הנדסית מקיפה לדירות ונכסים בעיר',
    soilType: 'קרקע חולית וקרקע חמרה, מפלס מי תהום גבוה יחסית באזורי החוף (מאתגר לאיטום מרתפים ויסודות)',
    buildingStyle: 'בנייני "בלוקים" משנות ה-60-70, פרויקטי תמ"א 38 וחיזוק סייסמי, מגדלי יוקרה חדשים',
    article: `כשניגשים לבדק בית בתל אביב, האתגר המרכזי הוא הגיל הממוצע הגבוה של המלאי הבנוי בעיר. חלק ניכר מהבניינים בתל אביב נבנו בשנות ה-60 וה-70, ורבים מהם עברו או עוברים כיום פרויקט תמ"א 38 לחיזוק סייסמי ותוספת בנייה. הבדיקה שלנו מתמקדת בנקודת המפגש הקריטית בין השלד הישן לתוספת החדשה - אזור נפוץ לכשלי איטום ולסדקים הנובעים מהפרשי התנהגות בין החומרים הישנים לחדשים.

בנוסף, בשל קרבת העיר לים והמפלס הגבוה יחסית של מי התהום באזורי החוף, אנו נותנים דגש מיוחד לאיטום מרתפים, קומות קרקע וחניונים תת-קרקעיים, שבהם רטיבות עולה (רטיבות קפילרית) היא תופעה שכיחה במיוחד. גגות שטוחים, האופייניים לבנייה הוותיקה בעיר, מהווים מוקד נוסף לכשלי איטום שיש לבדוק ביסודיות, בייחוד לקראת עונת החורף.

המומחים שלנו באריקס ביקורת מבנים מספקים דוחות הנדסיים מפורטים הכוללים צילומים תרמיים ואומדן עלויות לתיקון הליקויים. אנו מלווים את לקוחותינו בתל אביב מול קבלנים, יזמי תמ"א 38 ומוכרי דירות יד שנייה, כדי להבטיח שכל ליקויי הבנייה יתוקנו לשביעות רצונם המלאה ובהתאם לתקנים הישראליים.`,
    points: [
      'בדיקת נקודת המפגש בין שלד ישן לתוספת תמ"א 38',
      'אבחון איטום מרתפים, קומות קרקע וחניונים מול מי תהום גבוהים',
      'איתור רטיבות קפילרית וביקורת גגות שטוחים',
      'בדיקת מערכות אלומיניום ומעטפת בבניינים ותיקים ומגדלים'
    ],
    commonDefects: [
      { title: 'כשלי איטום במפגש תמ"א 38', desc: 'סדקים וחדירת מים בחיבור בין הבנייין הוותיק לתוספת הבנייה החדשה.' },
      { title: 'רטיבות קפילרית במרתפים', desc: 'חדירת מי תהום וחוסר באיטום שלילי בקומות תת-קרקעיות ומרתפים.' },
      { title: 'כשלי איטום בגגות שטוחים', desc: 'שחיקת יריעות ביטומניות בבנייה הישנה האופיינית לתל אביב.' }
    ],
    localStats: [
      { label: 'בדיקות שבוצעו בעיר', value: '650+' },
      { label: 'בעיות רטיבות במרתפים וקומות קרקע', value: '25%' }
    ],
    relatedServices: [
